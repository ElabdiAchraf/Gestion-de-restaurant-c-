...
namespace Restaurant.AllUserControl
{
    public partial class UC_UpdateItems : UserControl
    {
        ...
        {
            query ="select * from items where name_item like '"+txtSearchItem.Text+"%'";
            DataSet ds = fn.getData(query);
            guna2DataGridView1.DataSource = ds.Tables[0];
        }
        int id;
        ...
        {
            query = "update items set name_item ='" + txtItemName.Text + "',category='" + txtCategory.Text + "',price=" + txtPrice.Text + "where id_item =" + id + "";
            fn.setData(query);
            loadData();
